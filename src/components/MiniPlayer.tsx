...
const MiniPlayer: FC<IProps> = ({ song, setFullScreen, dispatch }) => {
  ...
  return (
    <div className="fixed left-0 bottom-0 z-[1000] flex h-16 w-full items-center bg-highlight_background_color">
      <div className="mr-2.5 ml-3 h-[48px] w-12" onClick={toggleToNormalPlayer}>
        <ProgressCircle radius={50} percent={0.2}>
          <img
            src={song.al.picUrl}
            alt={`${song.name}播放中`}
            className="absolute top-[4px] left-[4px] z-50 h-[42px] w-[42px] animate-mini-rotating rounded-full"
          />
        </ProgressCircle>
      </div>
